...
class TelemetryEvent(ContractModel):
    ...
    # for example after 2 month we add oil condition or angle,
    # on that time we add schema v2
    ...
    def measured_at_must_include_timezone(cls, value: datetime) -> datetime:
        """Reject timestamps without explicit timezone information."""
...
class TelemetryBatchRequest(ContractModel):
    ...
    # i want to concider max as 500 at first,
    # but after load test we may should change it.
